...
import type { ActivityPage } from "@/services/api/board-rows.api";
...
async function searchRows(
  ...
  // Backend thật trả rỗng cho truy vấn rỗng — `rows/search` là tìm kiếm, không
  // phải liệt kê. Fake này từng rộng lượng hơn và trả về tất cả, nên picker
  // relation xanh trên offline rồi mở ra trống trơn trên bản chạy thật.
  ...
  for (const rowId of record.order) {
    ...
    if (matchesRowId(row.displayId, query) || label.includes(needle)) {
      matches.push(row);
    }
  }
...
async function createColumn(
  ...
  // Config đi cùng lần tạo — một cột relation ra đời đã biết board đích của nó.
  // Merge chứ không thay: `makeColumn` đã đặt mặc định cho từng kiểu, và caller
  // chỉ gửi những field nó thật sự chọn.
  const column = (
    config === undefined ? base : { ...base, config: { ...base.config, ...config } }
  ) as BoardColumn;
...
/**
 * Server thật trả về một TRANG — `{ items, nextCursor }` — chứ không phải mảng
 * trần. Fake từng trả mảng, nên mọi test offline xanh trong khi app thật nổ
 * "entries is not iterable". Fake phải nói đúng hình dạng dây, kể cả khi chưa
 * ai dùng tới nextCursor.
 */
async function listActivity(
  ...
  return {
    items: [...record.activity.filter((entry) => entry.rowId === rowId), ...seeded],
    nextCursor: null,
  };
}
...
/**
 * Nhập một bảng tính — nhận FILE, đúng như backend thật.
 *
 * Fake này từng nhận sẵn `rows` đã dựng, nên bộ offline xanh trong khi endpoint
 * thật (multipart, E-085) trả 400 cho mọi lần bấm Import. Ở đây nó làm đúng
 * năm việc mà server làm: đọc file, dựng cột mới mà ánh xạ yêu cầu, bỏ cột bị
 * loại, ghép ô, rồi chèn record. Mã hiển thị vẫn do bộ đếm của board cấp.
 */
async function importRows(
  ...
  const extension = file.name.toLowerCase().split(".").pop() ?? "csv";
  const grid =
    extension === "xlsx"
      ? (await parseXlsx(new Uint8Array(await file.arrayBuffer()))).rows
      : parseDelimited(await file.text(), delimiterFor(extension));

  const source = readImportSource({ fileName: file.name, grid, hasHeaderRow });

  // Cột mới TRƯỚC: một record không mang nổi giá trị cho cột chưa tồn tại.
  const realIdBySourceIndex = new Map<number, string>();

  for (const draft of newColumnDrafts(mappings)) {
    const column = makeColumn(
      nextId("col"),
      uniqueColumnName(record.board.columns, draft.name.trim()),
      draft.type,
      record.board.columns.length,
    );

    record.board = {
      ...record.board,
      columns: [...record.board.columns, column],
      updatedAt: nowIso(),
    };
    realIdBySourceIndex.set(draft.sourceIndex, column.id);
  }

  const plan = planImport({
    source,
    mappings,
    columns: record.board.columns,
    context: { people: new Map(), relationLabels: new Map(), relationResolved: true },
  });

  const drafted = rowsToCreate(plan, invalidPolicy, planColumns(mappings, record.board.columns));
  ...
  // Cột bị loại đi SAU CÙNG, và chỉ khi record đã vào an toàn: một lần nhập
  // hỏng không được là thứ dọn sạch schema của board.
  const removedColumns: string[] = [];

  for (const columnId of removeColumnIds ?? []) {
    const column = record.board.columns.find((candidate) => candidate.id === columnId);
    if (!column || isProtectedColumn(column)) continue;

    record.board = {
      ...record.board,
      columns: removeColumn(record.board.columns, columnId),
      updatedAt: nowIso(),
    };
    removedColumns.push(column.name);
  }
  ...
  return {
    ...
    ...(removedColumns.length === 0 ? {} : { removedColumns }),
  };
}
...
async function listBoards(signal?: AbortSignal): Promise<readonly BoardDescriptor[]> {
  ...
  return boardNodes().map((node) => ({
    ...
    nodeId: node.id,
    name: node.name,
    // Cùng nguồn với board thật dựng ra từ node này, nên hai đường không lệch.
    rowIdPrefix: prefixForBoard(node.name, node.templateId),
  }));
}
...
/** Giải một tập row id thành mã và tiêu đề, kể cả khi chúng ở board khác. */
async function relationIndex(
  ...
  // GIẢI những id được hỏi, không liệt kê board. Id có thể thuộc board bất kỳ —
  // đó là điểm chính của cross-board relation — nên phải quét, và `boardId` chỉ
  // là chỗ đứng của người gọi (backend thật kiểm quyền từ đó).
  void boardId;

  const wanted = new Set(ids);

  return boardNodes().flatMap((node) => {
    const record = recordFor(node.id);
    const primaryId = record.board.primaryColumnId;

    return record.order.flatMap((rowId) => {
      if (!wanted.has(rowId)) return [];
