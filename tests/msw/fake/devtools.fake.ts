...
import type { SecretAuditPage } from "@/services/api/devtools.api";
...
import { auditFake } from "./audit.fake";
...
async function listSecretAudit(
  ...
  await readDelay(signal);
  return { items: [...secretRecord(nodeId).audit], nextCursor: null };
}
